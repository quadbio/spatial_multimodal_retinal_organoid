data.path = "/links/groups/treutlein/DATA/imaging/charmel/publication_example_data_2/"
proj.path = '/home/pwahle/4i_publication_repo_2'
library(reticulate)
...
yamlpath = '/home/pwahle/4i_publication_repo_2/metadata.yml'
colnames = read_yaml(yamlpath)
...
saveRDS(fSOM, file = paste0(data.path, '/fSOM_output/fSOM.rds'))
som_codes = fSOM$map$codes
np$savez_compressed(paste0(data.path, '/fSOM_output/som_codes.npz'), som_codes)
} else {
  fSOM = readRDS(paste0(data.path, '/fSOM_output/fSOM.rds'))
  som_codes = fSOM$map$codes
}
...
png(file=paste0(data.path, '/fSOM_output/kneeplot.png'), width=600, height=350)
...
print(df)
print(paste0('Choose k nearest neighbors to use for phenograph fom kneeplot in ', data.path, '/fSOM_output. Or choose from table printed above, or use default of 15 neighbors to get 31 MTUs'))

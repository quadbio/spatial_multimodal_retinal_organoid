...
def simple_mask(point, dir_input, dir_output, cycle='cycle1', save=True, plot=False, show_plot=False, save_plot=False):
  ...
  if not os.path.isfile(str(Path(dir_output,'simple_initial_mask'+str(point)+'.tif'))):
    ...
    filename = img_df.loc[(img_df['well_id'] == point) & (img_df['cycle_id'] == cycle) & (img_df['stain'] == 'hoechst')]['file'].values[0]
    ...
    if save:
        ...
        io.imsave(str(Path(dir_output,'simple_initial_mask'+str(point)+'.tif')), img_as_uint(img), check_contrast=False)
...
def denoise_well(well = 0):
    ...
    if not os.path.isfile(data_path + 'masks/cropped_simple_mask'+ str(well) +'.tif'):
        ...
        io.imsave(data_path + 'masks/cropped_simple_mask'+ str(well) +'.tif' , cropped_mask.astype(int), check_contrast=False)
...
def refine_mask(well = 0):
    ...
    if not os.path.isfile(data_path + '/masks/cropped_refined_mask' + str(well) + '.tif'):
        ...
        filenames = img_df.loc[(img_df['well_id'] == well) & (img_df['cycle_id'].isin([1,2,3,4,5,7,8,9,10,11]))]['file'].to_list()
        filenames = sorted_nicely(filenames)
        for i in filenames:
        ...
        io.imsave(data_path + '/masks/cropped_refined_mask' + str(well) + '.tif', mask3.astype('uint8'), check_contrast=False)
#################-----------------------------------------------------------------########################
#remove well and cycle specific backgrounds that are proportionally composed by combining the previous and consecutive elution cycle for every image per channel. Also move the reference dapi image to the same folder because this folder will have all the fully processed images used in the analysis
def bg_subtraction(well,
    ...
    file_dapi = df[(df['well_id'] == well) & (df['stain'] == 'hoechst') & (df['cycle_id'] == cycle_hoechst)]['file'].values[0]
    ...
    if not (all(os.path.isfile(i) for i in items)):
        for file_image in tqdm(files_images, ascii=True):
            if not os.path.isfile(Path(dir_output,file_image)):
                ...
                file_leading_cycle_bg = df_bg[(df_bg['cycle_id'] == leading_cycle_bg) & (df_bg['channel'] == channel)]['file'].values[0]
                img_bg_leading = io.imread(Path(dir_input, file_leading_cycle_bg))
                 
                #read lagging elution cycle
                lagging_cycle_bg = cycles_sorted[1]
                file_lagging_cycle_bg = df_bg[(df_bg['cycle_id'] == lagging_cycle_bg) & (df_bg['channel'] == channel)]['file'].values[0]
...
def segment_nuclei(well, 
    ...
    if not os.path.isfile(data_path + 'segmented_nuclei/' + str(well) + '.tif'):
        df = get_metadata(dir_input)
        filename = df[(df['cycle_id'] == ref_cycle) & (df['well_id'] == well) & (df['stain'] == 'hoechst')]['file'].values[0]
...
#################-----------------------------------------------------------------########################
#generate sample specific pixel matrices based on raw pixel intensities of the fully processed images. Optionally remove artifacts that reside in the collagen rich areas.
...
def get_collagen_mask(well, colname):
    ...
    return colmask   
#################-----------------------------------------------------------------########################
#normalize mtrices to maintain inter condition differences and reduce intra condition differences. Also generate downsampled, normalized and scaled fusion matrix to generate the reference for MTU clustering and scale individual matrices in the same way to allow MTU assignment to sample pixels
def run_matrix_normalization(wells, 
    ...
    if not (all(os.path.isfile(i) for i in items)):
        ...
        np.savez_compressed(Path(dir_output, 'scaled_fusion_sub250_z_normalised_pixel_matrix.npz'), scaled_long_matrix.astype('float32'))
        
        #scale and save sample matrices
        for i in np.arange(len(matrices)):
            mat = scale_matrix(normalized_mats[i],bottom_percentiles,top_percentiles)
            np.savez_compressed(Path(dir_output, str(wells[i])+'scaled_z_normalised_pixel_matrix.npz'), mat.astype('float32'))
...
def MTU_assignment(well = 43, 
    ...
    for cluster in cluster_labels_unique:
        idx = np.where(pixel_cluster_labels == cluster)[0]  # tuple where the second element is empty, so only take the first
    ...
    cm.ax_row_dendrogram.set_position([row_dend.x0+row_dend.width*0.5, row_dend.y0, row_dend.width*0.5, row_dend.height])
    # Get color bar
...
def show_staining(point, stain, dir_input='/links/groups/treutlein/DATA/imaging/PW/4i/plate14/', show_nucleus=False, nucleus_cycle=1):
  import os
  from pathlib import Path
  import pandas as pd
  import matplotlib.pyplot as plt
  ...
  timepoints = pd.read_csv('/links/groups/treutlein/DATA/imaging/PW/4i/plate14/accessory_data/conditions_per_sample.txt')
  ...
  if show_nucleus:
    ...
    plt.suptitle(' '.join(["Point:", str(point), "Stain:", "Hoechst", "from", cycle, timepoints.iloc[point].values[0]]), size=20)
  else:
    img = scale_image(img[...,channels[channel]])
    plt.suptitle(' '.join(["Point:", str(point), "Stain:", stain, "from", cycle, timepoints.iloc[point].values[0]]), size=20)

  plt.imshow(img)
  plt.show()

def make_collage(point, stains, dir_input='/links/groups/treutlein/DATA/imaging/PW/4i/plate14/', nucleus_cycle=1, crop=False, x=None, y=None, size=1000):
  import os
  from pathlib import Path
  import pandas as pd
  import matplotlib.pyplot as plt

...
# load functions
source('integration_4i_sc_rna_atac/utils.R')
...
get_distance_to_contour <- function(ids, x, y, df_contours) {
  ...
  apply(positions, 1,
        function(x) { apply(M, 1, euclidean, b = x) %>%
          min() %>%
          unique() }
  )
}
...
add_distance_to_contour <- function(result.list) {
  points <- result.list$seu_4i@meta.data %>%
    distinct(organoid) %>%
    pull(organoid)
  names(points) <- points
  message(result.list$seu_4i@meta.data %>%
            distinct(age) %>%
            pull())
  # setup of the future
  ...
  df_distances <- future_map(points, calculate_distances_to_contour, df_nuclei = result.list$seu_4i@meta.data, df_contours = df_contours) %>%
    bind_rows() %>%
    select(distance_to_contour)

  result.list$seu_4i@meta.data %>%
    mutate(rowname = rownames(.)) %>%
    left_join(df_distances %>% mutate(rowname = rownames(.)), by = 'rowname') -> df
...
add_wedge_dpt_assay <- function(result.list, wedge_ids, df_meta, df_pseudotime) {
  ...
  df_pseudotime_assay <- map(mapped_nuclei, function(x) { rownames_to_column(x, var = 'cell_id') }) %>%
    bind_rows(.id = 'wedge_id') %>%
    as_tibble() %>%
    group_by(dpt_rank, cell_id) %>%
    summarise(n = n())
  cells_in_window <- df_pseudotime_assay %>%
    distinct(cell_id) %>%
    pull()
  cells_not_in_window <- rownames(result.list$seu_4i@meta.data)[!(rownames(result.list$seu_4i@meta.data) %in% cells_in_window)]

  df_pseudotime_assay <- df_pseudotime_assay %>%
    pivot_wider(id_cols = dpt_rank, names_from = cell_id, values_from = n) %>%
    ungroup() %>%
    mutate_all(~replace(., is.na(.), 0))
  df_pseudotime_assay[, cells_not_in_window] <- 0

  M <- df_pseudotime_assay %>%
    select(-dpt_rank) %>%
    as.matrix()
  rownames(M) <- df_pseudotime_assay %>%
    select(dpt_rank) %>%
    pull() %>%
    as.character()
...
list_4i_nuclei_updated <- map(list_4i_nuclei, function(x) { x %>%
  structure_meta_data() %>%
  add_wedge_dpt_assay(wedge_ids = wedge_ids, df_meta = df_meta, df_pseudotime = df_pseudotime) %>%

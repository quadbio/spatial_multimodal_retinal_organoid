...
dir_output = 'data/processed/fish/laminator/results_fish'
...
for file in files:
    ...
    for i, transcript in tqdm(dict_transcripts.items(), ascii=True):
        df_intensity_profile[df_intensity_profile['stain'] == transcript].to_csv(
            Path(dir_results_point, 'df_intensity_profiles_{}.csv'.format(transcript)), index=False)

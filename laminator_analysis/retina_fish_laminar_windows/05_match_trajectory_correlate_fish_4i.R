source('~/4i_organoid_pipeline/retina/rotator_analysis/retina_fish_windows/0_utils_compare.R')
...
df_contours <- read_csv(str_c(dir_rotator,'df_contours.csv',sep='/'))
dir_dataset <- '/local2/USERS/charmel/rotator_dataset_fish_2'
...
list_4i_nuclei <-readRDS('/local2/USERS/charmel/integrated_seurat_4i_scRNA_scATAC_rev_2.rds')
...
for(i in seq_along(timepoints)){
  ...
  features <- seu_fish@assays$integrated@data %>% rownames() %>% set_names()
  fish_windows <- df_meta %>% filter(type==timepoints[i]) %>% distinct(id) %>% pull() %>% set_names()
  ...
  df_4i_average <- df_4i_windows %>% group_by(age, cell_type) %>% summarise_at(features, mean) %>% ungroup()%>% mutate(type=str_c(age, cell_type,sep='_'))
  M_4i_average <- df_4i_average %>% select(-type, -age, -cell_type) %>% as.matrix()
  rownames(M_4i_average) <- df_4i_average$type
  df_fish_average <-  df_fish_windows %>% group_by(cell_type) %>% summarise_at(features, mean) %>% ungroup()
  M_fish_average <- df_fish_average %>% select(-cell_type) %>% as.matrix()
  rownames(M_fish_average) <- df_fish_average$cell_type
  cor_results <- cor(t(rbind(M_fish_average, M_4i_average)))
  df_cors <- cor_results[rownames(M_fish_average),rownames(M_4i_average)] %>% t() %>%
    as_tibble(rownames='type') %>% separate(type,c('age','cell_type'),sep = '_') %>%
  ...
  df_4i_windows  %>% group_by(age) %>% mutate(n_total=n()) %>% group_by(age, cell_type) %>% mutate(n=n(), weight=n/n_total)  %>% select(n, age, cell_type,n_total, weight) %>% distinct()  -> df_frac
  df_fish_windows %>% mutate(n_total=n()) %>% group_by(cell_type) %>% mutate(n=n(), weight=n/n_total)  %>% select(n,cell_type,n_total, weight) %>% distinct()  -> df_frac


  df_cors_sum <- df_cors %>% left_join(df_frac) %>% group_by(age) %>% summarise(mean_weighted = weighted.mean(cor,weight))

  # TODO add timepoint constraints!
  ...
  M_fish <- df_fish_windows %>% select(features) %>% as.matrix()
  rownames(M_fish) <- df_fish_windows %>% mutate(names = str_c(window, id, sep='_')) %>% pull(names)

  # matrix predicted rna transcripts 4i nuclei
  df_matched_transcriptome <- df_matched_nuclei %>% mutate(id_4i = as.numeric(id_4i)) %>%
    left_join(df_4i_windows %>% select(c(dpt_rank,ID,features)) %>% rename(id_4i=ID), by=c('dpt_rank','id_4i')) %>%
    mutate(names=str_c(fish_window,id_fish, sep = '_'))

  M_4i <- df_matched_transcriptome %>% select(features) %>% as.matrix()
...
saveRDS(results,'/local2/USERS/charmel/rotator_analysis_results_fish/matched_trajectory_correlation_reimaged_2.rds')
message('Done.')
